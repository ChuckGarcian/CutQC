...
def write_results (dirname, filename, results):
    ...
    with open(output_file_path, 'w') as file:
        file.write(f"Compute Time: {results}\n")
        file.write("PT Version, {} nodes".format(WORLD_SIZE))

def run(args):
    filename = "{}_{}_{}".format(args.circuit_type, args.circuit_size, args.max_subcircuit_width)
    full_path = "pickle_files/{}.pkl".format(filename)
...
if __name__ == "__main__":
    print("--- Beginning dist_run.py ---")
    parser = argparse.ArgumentParser(description='Optional app description')

    # Required named arguments
    parser.add_argument('--circuit_type', type=str, required=True, help='Type of the circuit')
